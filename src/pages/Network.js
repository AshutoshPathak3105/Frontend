...
const Network = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [reqRes, connRes, followersRes, followingRes] = await Promise.all([
    ...
    useEffect(() => {
        ...
        // Handle deep-linking from Profile page (e.g., /network?tab=followers)
        const params = new URLSearchParams(location.search);
        const tab = params.get('tab');
        if (tab && ['requests', 'connections', 'followers', 'following'].includes(tab)) {
            setActiveTab(tab);
        }
                    ...
                    {networkSearch.trim().length >= 2 ? (
                                    ...
                                    {searchResults.map(person => {
                                        ...
                                        return (
                                            <div key={person._id} className="connection-card">
                                                <div className="connection-cover"><div className="connection-cover-pattern" /></div>
                                                <div className="conn-avatar-wrap"><LocalAvatar u={person} size={68} /></div>
                                                ...
                                                <div className="connection-footer">
                                                    {status === 'connected' ? (
                                                        <button className="btn btn-primary btn-full" onClick={() => navigate('/messages')}><MessageCircle size={15} /> Message</button>
                                                    ) : status === 'pending' ? (
                                                        <button className="btn btn-outline btn-full" disabled><UserCheck size={15} /> Pending</button>
                                                    ) : (
                                                        <button className="btn btn-primary btn-full" onClick={() => handleConnect(person._id)} disabled={connecting[person._id]}>
                        ...
                        activeTab === 'requests' ? (
                            <div className="requests-grid">
                                {requests.length === 0 ? (
                                    <div className="empty-state">
                                        <div className="empty-icon-wrap"><UserPlus size={48} /></div>
                                        <h3>No pending requests</h3>
                                        <p>When people want to connect with you, their requests will appear here.</p>
                                        <Link to="/people" className="btn btn-primary">Find People</Link>
                                    </div>
                                ) : requests.map(req => (
                                    <div key={req._id} className="request-card">
                                        <div className="request-cover"><div className="connection-cover-pattern" /></div>
                                        <div className="conn-avatar-wrap"><LocalAvatar u={req.sender} size={68} /></div>
                                        <div className="conn-body">
                                        ...
                                        <div className="request-footer">
                                            ...
                                            <button className="btn btn-outline req-action-btn ignore-btn" onClick={() => handleRespond(req._id, 'rejected')}><X size={15} /> Ignore</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="connections-grid">
                                {connections.length === 0 ? (
                                    <div className="empty-state">
                                        ...
                                        <Link to="/people" className="btn btn-primary">Discover People</Link>
                                    </div>
                                ) : connections.map(conn => (
                                    <div key={conn._id} className="connection-card">
                                        <div className="connection-cover"><div className="connection-cover-pattern" /></div>
                                        <div className="conn-avatar-wrap"><LocalAvatar u={conn} size={68} /></div>
                                        <div className="conn-body">
                                        ...
                                        <div className="connection-footer">
                                            ...
                                            <div className="menu-container" style={{ position: 'relative' }}>
                                                <button className="btn btn-outline icon-only" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === conn._id ? null : conn._id); }}>
                                                    <MoreHorizontal size={16} />
                                                </button>
                                                {openMenuId === conn._id && (
                                                    <div className="network-dropdown-menu">
                                                        <button onClick={() => handleToggleFollow(conn._id)}>{user?.following?.includes(conn._id) ? <><UserCheck size={14} /> Unfollow</> : <><UserPlus size={14} /> Follow User</>}</button>
                                                        <button className="danger" onClick={() => handleRemoveConnection(conn._id)}><X size={14} /> Unfriend</button>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                            ...
                            <div className="connections-grid">
                                {followers.length === 0 ? (
                                    <div className="empty-state">
                                        <div className="empty-icon-wrap"><UserCheck size={48} /></div>
                                        <h3>No followers yet</h3>
                                        <p>Grow your following by engaging with others.</p>
                                    </div>
                                ) : followers.map(f => (
                                    <div key={f._id} className="connection-card">
                                        <div className="connection-cover"><div className="connection-cover-pattern" /></div>
                                        <div className="conn-avatar-wrap"><LocalAvatar u={f} size={68} /></div>
                                        <div className="conn-body">
