...
const LogoImage = ({ height = 42, withText = true, className = "" }) => {
    // Aspect ratio: Icon is roughly square(1:1), with text it's wider (approx 5.5:1)
    const width = withText ? height * 5.5 : height;

    return (
        <svg
            width={width}
            height={height}
            viewBox={withText ? "0 0 550 100" : "0 0 145 100"}
            fill="none"
        ...
        >
            <defs>
                ...
                <linearGradient id="sarthiGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor="#fbbf24" />
                    <stop offset="100%" stopColor="#f59e0b" />
                </linearGradient>
                {/* Soft shadow for the briefcase */}
                <filter id="shadow" x="-2" y="-2" width="120%" height="120%">
